const lang = {
  'en-US': {
    nav: {
      home: 'Home',
      account: 'Account',
    },
    index: {
      trending: 'Trending',
      now: 'Now',
      new: 'New',
      releases: 'Releases',
    },
    movie: {
      cast: 'Cast',
      crew: 'Crew',
      filmography: 'Filmography',
      born: 'Born',
      died: 'Died',
      from: 'From',
      trailer: 'Watch Trailer',
      info: 'Movie Info',
      production: 'Other production',
      none: 'Currently no other film',
      no_image: 'Currently no image, will fix it soon.',
    },
    save: 'Save',
    remove: 'Remove',
    account: {
      login: 'Log In',
      signup: 'Sign Up',
      collapse: 'Collapse',
      my_account: 'My Account',
      my_reviews: 'My Reviews',
      saved_reviews: 'Saved Reviews',
      saved_movies: 'Saved Movies',
      logout: 'Log out',
      username: 'username',
      email: 'email',
      update_image: 'Update Image',
      no_submitted_review: "You haven't submitted any reviews yet",
      no_saved_review: "You haven't saved any reviews yet",
      no_saved_movie: "You haven't saved any movies yet",
    },
    review: {
      submit_reivew: 'Submit Review',
      see_all_review: 'See All Review',
      title: 'Title',
      content: 'Content',
      image: 'Image',
      no_review: 'Currently no review',
      spoiler: 'Spoiler Alert',
    },
    submit: 'Submit',
    upload: 'Upload',
    edit: 'Edit',
    delete: 'Delete',
    update: 'Update',
    cannot_find: "Cannot Find the Movie You're Searching For",
    404: 'Looks like the page you were looking for is no longer here',
  },
  'zh-TW': {
    nav: {
      home: '首頁',
      account: '帳號',
    },
    index: {
      trending: '熱門',
      now: '電影',
      new: '最新',
      releases: '上映',
    },
    movie: {
      cast: '演員',
      crew: '工作人員',
      filmography: '其他演出作品',
      born: '出生',
      died: '過世',
      from: '來自',
      trailer: '預告片',
      info: '電影資訊',
      production: '其他製作作品',
      none: '目前無資訊',
      no_image: '目前無圖片，將盡速更新',
    },
    save: '收藏',
    remove: '移除',
    account: {
      login: '登入',
      signup: '註冊',
      collapse: '收合',
      my_account: '我的帳號',
      my_reviews: '我的評論',
      saved_reviews: '收藏評論',
      saved_movies: '收藏電影',
      logout: '登出',
      username: '使用者名稱',
      email: '電子信箱',
      update_image: '更新頭像',
      no_submitted_review: '目前沒有上傳的評論',
      no_saved_review: '目前沒有收藏的評論',
      no_saved_movie: '目前沒有收藏的電影',
    },
    review: {
      submit_reivew: '上傳評論',
      see_all_review: '看所有評論',
      title: '標題',
      content: '內容',
      image: '圖片',
      no_review: '目前沒有評論',
      spoiler: '防雷提示',
    },
    submit: '提交',
    upload: '上傳',
    edit: '編輯',
    delete: '刪除',
    update: '更新',
    cannot_find: '找不到此電影',
    404: '頁面不存在',
  },
  'fr-FR': {
    nav: {
      home: 'Accueil',
      account: 'Mon compte',
    },
    index: {
      trending: 'En',
      now: 'Salles',
      new: 'Dernières',
      releases: 'News',
    },
    movie: {
      cast: 'Acteurs et Actrices',
      crew: "l'équipe",
      filmography: 'Filmographie',
      born: 'Né',
      died: 'Mort',
      from: 'De',
      trailer: 'Bandes-Annonces',
      info: 'Info de Film',
      production: 'Les autre productions',
      none: 'Actuellement aucun autre film',
      no_image: "Actuellement pas d'image, va le corriger bientôt.",
    },
    save: 'Envie de voir',
    remove: 'Retirer',
    account: {
      login: 'Se connecter',
      signup: "S'inscrire",
      collapse: 'Effondrement',
      my_account: 'Mon compte',
      my_reviews: 'Mes commentaires',
      saved_reviews: 'Commentaires Enregistrés',
      saved_movies: 'Films Enregistrés',
      logout: 'Se déconnecter',
      username: 'Nom',
      email: 'e-mail',
      update_image: "Réviser l'image",
      no_submitted_review: "Vous n'avez pas encore soumis d'un commentaire",
      no_saved_review: "Vous n'avez pas encore enregistré d'un commentaire",
      no_saved_movie: "Vous n'avez pas encore enregistré de films",
    },
    review: {
      submit_reivew: 'Soumettre',
      see_all_review: 'Voir tous les commentaires',
      title: 'Titre',
      content: 'Contenu',
      image: 'Image',
      no_review: 'Pas de commentaire',
      spoiler: 'Alerte Spoiler',
    },
    submit: 'Soumettre',
    upload: 'Télécharger',
    edit: 'Éditer',
    delete: 'Effacer',
    update: 'Réviser',
    cannot_find: 'Impossible de trouver le film que vous recherchez',
    404: "La page que vous recherchez n'existe plus",
  },
};

module.exports = lang;
